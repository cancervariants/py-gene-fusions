# Unit Testing for pyGeneFusion class
import pytest
from test import *
...
def test_chromosome_location():
    ...
    c1 = ChromosomeLocation(species = 'taxonomy:4232', chr = '5', start = 'p13.2', end = 'p13.5')
    ...
    with pytest.raises(AssertionError):
        ChromosomeLocation(species ='taxonomy4232', chr=5, start='s13.2', end= 14)
...
def test_regulatory():
    ...
    reg1 = RegulatoryElement(type = 'promoter', value_id = 'hgnc:435', label = 'G1')

    with pytest.raises(AssertionError):
        RegulatoryElement(type = 'notpromoter', value_id = 'hgnc435', label = 5)

def test_genomic_region_seq():
    """Test GenomicRegion object, with SequenceLocation, initializes correctly"""
    g1 = GenomicRegion(value= SequenceLocation(type = 'SequenceLocation', seq_id = 'ncbi:NC_000001.11',  start =  154170399, end = 1556346346))
    ...
    with pytest.raises(AssertionError):
        GenomicRegion(value = SequenceLocation(type='GeneLocation', seq_id='ncbiNC_000001.11', start='154170399', end='1556346346'))


def test_genomic_region_chr():
    """Test GenomicRegion object, with ChromosomeLocation, initializes correctly"""
    g1 = GenomicRegion(value = ChromosomeLocation(type = 'ChromosomeLocation', species = 'taxonomy:9606' ,chr = '12',
    ...
    with pytest.raises(AssertionError):
        GenomicRegion(value = ChromosomeLocation(type='GeneLocation', species='taxonomy9606', chr=2, start='s12.1',
                                         end='s12.2'))
...
def test_transcript_component():
    ...
    tr1 = TranscriptComponent(transcript = 'nm:152263.3', exon_start = 1, exon_start_offset = -9,
                               exon_end = 8, exon_end_offset = 7, gene = gen1, component_genomic_region = g1)
    ...
    with pytest.raises(AssertionError):
        TranscriptComponent(transcript = 'nm152263.3', exon_start = '1', exon_start_offset = '-9',
...
def test_critical_domain():
    ...
    with pytest.raises(AssertionError):
        CriticalDomain(status='gained', name= 'tyrosine kinase catalytic domain', id='interproIPR020635',
                            gene= 7)
...
def test_fusion():
    ...
    try:
        Fusion(r_frame_preserved = True, regulatory_elements = [reg1, reg2], protein_domains = [domain],
                                  transcript_components = [c1, c2, tr1, linker_seq, tr2], causative_event = cause_eve)
    except AssertionError:
        pass

    with pytest.raises(AssertionError):
        Fusion(r_frame_preserved= 'T', regulatory_elements=[5, 5], protein_domains=[7,7],
